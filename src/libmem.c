...
int __alloc(struct pcb_t *caller, int vmaid, int rgid, int size, int *alloc_addr)
{
  ...
  /* Define page count calculation if not available */
  #ifndef PAGING_PAGE_COUNT
  #define PAGING_PAGE_COUNT(sz) ((sz + PAGING_PAGESZ - 1) / PAGING_PAGESZ)
  #endif
...
int pg_setval(struct mm_struct *mm, int addr, BYTE value, struct pcb_t *caller)
{
  ...
  regs.a2 = phyaddr;                 // Physical address
  regs.a3 = (uint32_t)value;         // Data to write (passed by value)
